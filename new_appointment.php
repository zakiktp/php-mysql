...
$last_id_stmt = $pdo->query("SELECT MAX(patient_id) FROM patients");
$last_id = $last_id_stmt->fetchColumn(); // e.g., 'AH0023'

if ($last_id) {
    $numeric_part = intval(substr($last_id, 2)); // extract number after 'AH'
    $next_numeric = $numeric_part + 1;
    $generated_patient_id = 'AH' . str_pad($next_numeric, 4, '0', STR_PAD_LEFT); // e.g., 'AH0024'
} else {
    ...
    <div class="card p-3">
            ...
            <div class="form-row">
                ...
                <div class="form-group col-md-4">
                    ...
                    <input type="text" name="staff" id="staff" class="form-control" value="<?= $staff_username ?>" readonly>
                </div>
            </div>
...
function addNewCity() {
  ...
  let newCity = input.value.trim().toUpperCase();
  if (newCity === "") {
